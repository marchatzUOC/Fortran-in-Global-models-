...
INTEGER(KIND=JPIM) :: JL, ID, JAER, INBAER,jp
...
!!$         cult(JL)   = 0.
   ...
   ! END IF ! newday 
         ...
         IF( Ustar > 0.0_JPRB .AND. (Ustar > umin2(JL) / c_eff(JL)) ) THEN
          !>>> TvN 
            ...
            DO WHILE( dp <= Dmax+1.0E-5_JPRB)
               ...
               PAERFLX(JL,10,6) = SREL(i_s1,1)
               PAERFLX(JL,10,7) = SREL(i_s1,48)
               PAERFLX(JL,10,8) = SREL(i_s1,96)
               PAERFLX(JL,10,9) = SREL(i_s1,192)
    ...
    ! ============================================================
    ! PAERFLX page 8: local inputs / masks
    ! ============================================================
    PAERFLX(JL,8,1) = PLSM(JL)
    PAERFLX(JL,8,2) = PWIND(JL) * 100.0_JPRB
    PAERFLX(JL,8,3) = PSNS(JL)
    PAERFLX(JL,8,4) = SOIL_TYPE(JL)
    PAERFLX(JL,8,5) = POT_SOURCE(JL)
    PAERFLX(JL,8,6) = snowcover(JL)
    PAERFLX(JL,8,7) = desert(JL)
    PAERFLX(JL,8,8) = lai_eff(JL)
    PAERFLX(JL,8,9) = Z0(JL)
    
    ! ============================================================
    ! PAERFLX page 9: threshold / correction state
    ! ============================================================
    PAERFLX(JL,9,1) = umin2(JL)
    PAERFLX(JL,9,2) = alpha(JL)
    PAERFLX(JL,9,3) = c_eff(JL)
    PAERFLX(JL,9,4) = Ustar
    PAERFLX(JL,9,5) = rho_air
    PAERFLX(JL,9,6) = airdens_ratio
    PAERFLX(JL,9,7) = airdens_ratio2
    PAERFLX(JL,9,8) = FPAR(JL)
    PAERFLX(JL,9,9) = REAL(idust, JPRB)
    
    ! ============================================================
    ! PAERFLX page 10: sampled threshold/distribution diagnostics
    ! ============================================================
    PAERFLX(JL,10,1) = UTH(1)
    PAERFLX(JL,10,2) = UTH(48)
    PAERFLX(JL,10,3) = UTH(96)
    PAERFLX(JL,10,4) = UTH(144)
    PAERFLX(JL,10,5) = UTH(192)
    !PAERFLX(JL,10,6) = SREL(i_s1,1)
    !PAERFLX(JL,10,7) = SREL(i_s1,48)
    !PAERFLX(JL,10,8) = SREL(i_s1,96)
    !PAERFLX(JL,10,9) = SREL(i_s1,192)
    ...
    ! ============================================================
    ! PAERFLX page 11: traced-bin total flux before masking
    ! ============================================================
    PAERFLX(JL,11,1) = fluxtot(1)
    PAERFLX(JL,11,2) = fluxtot(2)
    PAERFLX(JL,11,3) = fluxtot(3)
    PAERFLX(JL,11,4) = fluxtot(4)
    PAERFLX(JL,11,5) = fluxtot(5)
    PAERFLX(JL,11,6) = fluxtot(6)
    PAERFLX(JL,11,7) = fluxtot(7)
    PAERFLX(JL,11,8) = fluxtot(8)
    PAERFLX(JL,11,9) = SUM(fluxtot(:))
    
    
    ! ============================================================
    ! PAERFLX page 12: masked bins and final ai/coi split
    ! ============================================================
    PAERFLX(JL,12,1) = fdust(1)
    PAERFLX(JL,12,2) = fdust(2)
    PAERFLX(JL,12,3) = fdust(3)
    PAERFLX(JL,12,4) = fdust(4)
    PAERFLX(JL,12,5) = flux_r1
    PAERFLX(JL,12,6) = flux_r2
    PAERFLX(JL,12,7) = flux_ai(JL)
    PAERFLX(JL,12,8) = flux_ci(JL)
    PAERFLX(JL,12,9) = SUM(fdust(:))
...
!WRITE(NULOUT,*) 'LAT DEG radians = ', PGLAT(KIDIA),PGLAT(KFDIA)
!WRITE(NULOUT,*) 'LON DEG radians = ', PGLON(KIDIA),PGLON(KFDIA)
...
!-- betwe!ien half-levels IHTST-1 and KLEV
